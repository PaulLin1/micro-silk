import { pgTable, integer, serial, jsonb, timestamp, text, uniqueIndex, index } from "drizzle-orm/pg-core";
...
export const connections = pgTable("connections", {
  ...
  uniqBlockChannel: uniqueIndex("uniq_block_channel").on(t.blockId, t.channelId),
...
export const holdoutConnections = pgTable("holdout_connections", {
  id: serial("id").primaryKey(),
  blockId: integer("block_id").notNull().references(() => blocks.id),
  channelId: integer("channel_id").notNull().references(() => channels.id),
  crawledAt: timestamp("crawled_at").defaultNow().notNull(),
}, (t) => ({
  uniqBlockChannel: uniqueIndex("uniq_holdout_block_channel").on(t.blockId, t.channelId),
}));